...
async function ensureCollection(env, uid) {
  ...
  if (!created.r.ok)
    throw new Error(created.data.error || "Unable to create work collection.");
  return created.data.collection;
}
async function records(env, uid, cid, q = "") {
  const x = await callStore(
    env,
    uid,
    `/data/collections/${cid}/records?limit=200${q ? "&q=" + encodeURIComponent(q) : ""}`,
  );
  if (!x.r.ok) throw new Error(x.data.error || "Unable to read work records.");
...
async function updateRecord(env, uid, cid, rid, name, data) {
  return callStore(env, uid, `/data/collections/${cid}/records/${rid}`, {
    method: "PUT",
...
export async function handleNativeWork(request, env, account) {
  ...
  if (!p.length) {
    ...
    if (request.method === "POST") {
      ...
      const d = {
        ...
        status: clean(b.status || "active", 40),
      };
      const x = await callStore(
        env,
        account.uid,
      ...
      return json({ project: normalizeProject(x.data.record) }, 201);
    }
    return json({ error: "Method not allowed." }, 405);
  }
  const projectId = p[0];
  if (!validId(projectId)) return json({ error: "Invalid project id." }, 400);
  const projectRecord = await getRecord(env, account.uid, cid, projectId);
  if (!projectRecord || projectRecord.data?.type !== "project")
    return json({ error: "Project not found." }, 404);
  if (p.length === 1) {
    if (request.method === "GET")
      return json({ project: normalizeProject(projectRecord) });
    if (request.method === "PUT") {
      const b = await readJsonLimited(request, 256 * 1024),
        old = projectRecord.data || {},
        name = clean(b.name || projectRecord.name),
        d = {
          ...old,
          type: "project",
          name,
          ...
          status: clean(b.status || old.status || "active", 40),
        };
      const x = await updateRecord(env, account.uid, cid, projectId, name, d);
      if (!x.r.ok) return json(x.data, x.r.status);
      return json({
        project: {
          id: projectId,
    ...
    if (request.method === "DELETE") {
      ...
      for (const t of all.filter(
        (r) => r.data?.type === "task" && r.data?.project_id === projectId,
      ))
        await callStore(
          env,
          account.uid,
          `/data/collections/${cid}/records/${t.id}`,
          { method: "DELETE" },
        );
      const x = await callStore(
        env,
        account.uid,
        `/data/collections/${cid}/records/${projectId}`,
        { method: "DELETE" },
      );
      if (!x.r.ok) return json(x.data, x.r.status);
      return json({ ok: true });
  ...
  if (p.length === 2) {
    ...
    if (request.method === "POST") {
      ...
      const d = {
        ...
        name,
        description: clean(b.description, 4000),
        status: clean(b.status || "todo", 40),
        priority: clean(b.priority || "normal", 40),
        due_at: clean(b.due_at || "", 80),
      };
      const x = await callStore(
        env,
        account.uid,
      ...
      if (!x.r.ok) return json(x.data, x.r.status);
      return json({ task: normalizeTask(x.data.record) }, 201);
    }
    return json({ error: "Method not allowed." }, 405);
  }
  const taskId = p[2];
  if (!validId(taskId)) return json({ error: "Invalid task id." }, 400);
  const taskRecord = await getRecord(env, account.uid, cid, taskId);
  if (
    !taskRecord ||
    taskRecord.data?.type !== "task" ||
    taskRecord.data?.project_id !== projectId
  )
    return json({ error: "Task not found." }, 404);
  if (request.method === "GET")
    return json({ task: normalizeTask(taskRecord) });
  if (request.method === "PUT") {
    const b = await readJsonLimited(request, 256 * 1024),
      old = taskRecord.data || {},
      name = clean(b.name || taskRecord.name),
      d = {
        ...old,
        type: "task",
        project_id: projectId,
        ...
        description:
          b.description === undefined
            ? old.description || ""
            : clean(b.description, 4000),
        status: clean(b.status || old.status || "todo", 40),
        priority: clean(b.priority || old.priority || "normal", 40),
        due_at: b.due_at === undefined ? old.due_at || "" : clean(b.due_at, 80),
      };
    const x = await updateRecord(env, account.uid, cid, taskId, name, d);
    if (!x.r.ok) return json(x.data, x.r.status);
    return json({
      task: {
        id: taskId,
  ...
  if (request.method === "DELETE") {
    const x = await callStore(
      env,
      account.uid,
      `/data/collections/${cid}/records/${taskId}`,
      { method: "DELETE" },
    );
    if (!x.r.ok) return json(x.data, x.r.status);
    return json({ ok: true });
