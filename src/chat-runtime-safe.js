...
const boundButtons=new WeakSet(),boundInputs=new WeakSet();let sending=false;
...
async function send(e){if(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}if(sending)return;const input=$('#prompt'),q=input&&input.value.trim();if(!q)return;sending=true;const btn=$('#sendBtn');if(btn)btn.disabled=true;const {a,s}=currentSession();const turn={q,at:Date.now(),answers:{},errors:{},synthesis:null,pending:true};s.turns=s.turns||[];s.turns.push(turn);if(!s.title)s.title=q.slice(0,80);s.updatedAt=Date.now();save([s,...a.filter(x=>x.id!==s.id)],s);input.value='';input.style.height='auto';notify();try{const ans=await askWorkers(s,q);turn.answers.workers=ans||'(empty response)'}catch(err){turn.errors.workers=String(err&&err.message||err);turn.answers.workers='Došlo je do problema pri odgovoru. Pokušaj ponovo.'}finally{delete turn.pending;s.updatedAt=Date.now();const list=load();save([s,...list.filter(x=>x.id!==s.id)],s);sending=false;const now=$('#sendBtn');if(now)now.disabled=false;notify()}}
function keyHandler(e){if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();send(e)}}
function bind(){const btn=$('#sendBtn'),input=$('#prompt');if(btn&&!boundButtons.has(btn)){boundButtons.add(btn);btn.addEventListener('click',send,true)}if(input&&!boundInputs.has(input)){boundInputs.add(input);input.addEventListener('keydown',keyHandler,true)}return !!(btn&&input)}
function boot(){bind();const mo=new MutationObserver(()=>bind());mo.observe(document.body,{childList:true,subtree:true});setInterval(bind,500)}
document.readyState==='loading'?document.addEventListener('DOMContentLoaded',()=>setTimeout(boot,10)):setTimeout(boot,10);
})();`;
